...
(function () {
  ...
  function updateCounter() {
    ...
    // The value is a machine-readable key sequence for AT. 'Meta' corresponds to Cmd/Win,
    // and 'Control' corresponds to Ctrl.
    const ariaMod = navigator.platform.includes('Mac') ? 'Meta' : 'Control';

    if (n > 0) {
      btnSelected.setAttribute('aria-keyshortcuts', `${ariaMod}+Enter`);
      btnAll.removeAttribute('aria-keyshortcuts');
      btnSelected.after(shortcutHintEl);
    } else {
      btnAll.setAttribute('aria-keyshortcuts', `${ariaMod}+Enter`);
