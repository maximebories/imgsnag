...
(function () {
  ...
  function updateCounter() {
    ...
    const dlSelectedKey = pluralRules().select(n) === 'one' ? 'popupDownloadSelectedCountOne' : 'popupDownloadSelectedCountOther';
    btnSelected.textContent = browser.i18n.getMessage(dlSelectedKey, [n.toString()]);

    const dlAllKey = pluralRules().select(total) === 'one' ? 'popupDownloadAllCountOne' : 'popupDownloadAllCountOther';
    btnAll.textContent = browser.i18n.getMessage(dlAllKey, [total.toString()]);
  ...
  function wrapCell(cell, item) {
    ...
    const fullDownloadLabel = browser.i18n.getMessage('popupDownloadFile', [`${fullFilename}${dimSuffix}`]);
    ...
    const fullSelectLabel = browser.i18n.getMessage('popupSelectFile', [optionName]);
    const fullDeselectLabel = browser.i18n.getMessage('popupDeselectFile', [optionName]);
