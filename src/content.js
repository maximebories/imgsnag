...
(function () {
  ...
  function collectImages(trackImage) {
    ...
    document.querySelectorAll('img[src], img[data-src], img[data-lazy-src], img[data-original], input[type="image"][src], input[type="image"][data-src], input[type="image"][data-lazy-src], input[type="image"][data-original]').forEach((img) => {
  ...
  function getTargetUrlsForElement(el) {
    ...
    if ((el.tagName === 'IMG' || (el.tagName === 'INPUT' && el.type === 'image')) && el.src) {
  ...
  document.addEventListener('dragend', (e) => {
    // Warden: Prevent hostile pages from synthesizing events to force downloads
    if (!e.isTrusted) return;
    if ((e.target.tagName === 'IMG' || (e.target.tagName === 'INPUT' && e.target.type === 'image')) && !isDragDisabled) {
      const attrs = ['data-src', 'data-lazy-src', 'data-original', 'src'];
      for (const attr of attrs) {
        let val;
