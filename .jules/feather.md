...
## 2026-08-26 - Regex fast-path for URL extraction
**Learning:** Applying complex regular expressions (like URL extractors) across thousands of DOM text nodes or attributes is CPU-intensive. By implementing a fast-path pre-check (`.includes('http') || .includes('HTTP') || .toLowerCase().includes('http')`), we bypass the regex engine entirely for the vast majority of non-matching strings, yielding significant main-thread CPU savings (e.g., from ~100ms down to ~80ms in benchmark).
**Action:** When applying regular expressions to search for URLs over large numbers of DOM strings (text nodes or attributes), implement a fast-path string `.includes` pre-check before executing the regex to bypass the engine for non-matching strings.
