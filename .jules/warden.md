...
## 2024-08-29 - Defense-in-depth: Centralized URL Validation and ZIP Hardening
**Vulnerability:** None (Enhancement). The sweep came back clean.
**Action:** Added threat-model comments at trust boundaries, tightened `build.sh` zip exclusions to prevent shipping dotfiles, and centralized the `http:`/`https:` protocol allowlist in `src/background.js` into a `getSafeDownloadUrl` helper for maintainability.
