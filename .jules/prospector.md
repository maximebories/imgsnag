...
**Learning:** The fundamental boundary for imgsnag is its delivery mechanism: a **URL handed to `browser.downloads.download`**. Any channel that does not terminate in a fetchable URL is architecturally incompatible, not merely expensive. This rules out channels that require heavy reassembly (like muxing raw binary WebSocket chunks) or non-URL-addressable asset storage.
**Action:** Use this architectural boundary as a primary filter to quickly discard future candidate channels that cannot produce a discrete URL for the download manager.
