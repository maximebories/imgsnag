...
## 2026-09-15 - JSON-LD structured data image extraction RFC
**Learning:** Extensionless media URLs embedded in JSON-LD structured data (e.g., schema.org metadata) are invisible to both the `IMAGE_URL_RE` text sweep (which requires file extensions) and `PerformanceObserver` (which only catches loaded resources). Structurally parsing `<script type="application/ld+json">` with `JSON.parse` is fast and safely recovers these high-quality, often untouched originals.
**Action:** Verdict GO, staged. Orchestrator filed RFC issue.
