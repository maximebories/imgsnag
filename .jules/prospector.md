...
## 2026-08-21 - Shadow DOM traversal
**Learning:** Open shadow roots hide embedded images (e.g. YouTube web components, widgets) from main document traversal. A recursive TreeWalker checking for `.shadowRoot` finds these elements, but iterating all nodes incurs a CPU cost that should be budgeted to the initial scan rather than continuous mutations.
**Action:** Verdict GO, staged. RFC issue written. Keep traversal out of the hot path.
