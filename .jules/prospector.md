...
**Learning:** `navigator.mediaSession.metadata` (specifically `.artwork`) contains high-quality media artwork URLs explicitly intended for OS integration by music and video players. These URLs often do not appear in the DOM (or only appear as low-res thumbnails). Since it is an isolated world issue, accessing it requires main-world execution or a script injection.
**Action:** Verdict GO, staged. Orchestrator will file the RFC. Keep as on-demand extraction to avoid polling overhead.
