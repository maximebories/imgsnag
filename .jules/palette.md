...
## 2024-08-04 - Screen Reader Pronunciation and Decorative Icons
**Learning:** Browser extension HTML pages without a dynamic `lang` attribute cause screen readers to struggle with correct pronunciation, and decorative nested SVGs inside semantic buttons can cause redundant announcements.
**Action:** Always inject `document.documentElement.lang = browser.i18n.getUILanguage();` on load, and explicitly set `aria-hidden="true"` on purely decorative SVGs within labeled parent elements.
