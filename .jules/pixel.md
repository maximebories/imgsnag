...
## 2024-08-01 - Dynamic Screen Reader Context
**Learning:** Browser extension pages often lack proper AT document context. Static `lang` attributes on `<html>` fail for localized extensions, and missing `document.title` causes screen readers to read the raw extension URL (e.g., `chrome-extension://.../options.html`).
**Action:** Always inject `document.documentElement.lang = browser.i18n.getUILanguage();` and localize `document.title` dynamically in script initialization.
