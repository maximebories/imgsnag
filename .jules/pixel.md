...
## 2026-09-15 - Interactive State Specificity
**Learning:** `.cell .check:hover` weighs **(0,2,0)** — strictly lower than `.selected`. A `.selected` checkbox had no hover feedback at all because the `.selected` state class simply outweighed the baseline pseudo-class, rather than due to source ordering.
**Action:** When styling interactive states that combine state classes (`.selected`) and pseudo-classes (`:hover`, `:focus-visible`), ensure explicit combined selectors (e.g., `.selected:hover`, `.selected:focus-visible`) are defined so that they have sufficient specificity to override the baseline state class.
