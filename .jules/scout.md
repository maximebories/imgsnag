...
**Learning:** Modern web pages use `<link rel="preload" as="image" imagesrcset="...">` to preload responsive images. The `imagesrcset` attribute lists multiple sizes, and the standard `href` is just a fallback. Previously, `handleMeta` only read the `href` attribute and completely ignored `imagesrcset`.
**Action:** Added targeted extraction for `imagesrcset` alongside `srcset`, `data-srcset`, and `data-bgset` in the structural scan (`collectImages`) and the MutationObserver path (`handleSrcset`). Also updated `handleMeta` to parse `imagesrcset` using `pickBestFromSrcset` on `<link rel="preload" as="image">` and return early to skip the `href` fallback, mirroring how `<picture>` sources and `<img srcset>` avoid duplicates.
