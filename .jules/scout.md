...
## 2026-09-07 - Widening parser input surface creates new false positives
**Learning:** When writing a parser to capture more text (e.g., expanding `IMAGE_SET_RE` to correctly traverse parenthesis depth to capture bare-string entries after `url()`), any other syntax allowed within that wider text is now exposed. In `image-set()`, the `type("image/avif")` descriptor contains quotes, which were incorrectly extracted as image URLs because they were no longer truncated.
**Action:** When you widen a parser's input window, explicitly re-check what else that window now contains and ensure it is properly stripped or ignored. Additionally, when writing regression tests, add new cases alongside existing ones instead of replacing them to preserve coverage of both positive and negative orderings.
