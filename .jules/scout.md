...
**Action:** Replaced the short-circuiting logic with a loop that extracts and tracks all available lazy-load attributes. The underlying Sets automatically deduplicate identical URLs, and the size/network filters ensure placeholders are dropped while preserving the actual high-quality media.
