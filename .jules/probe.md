...
## 2026-09-18 - resolveUrl allowlist protocol tests
**Learning:** Verified that `resolveUrl` explicitly whitelists `http:`, `https:`, `blob:`, and `data:` protocols in `src/content.js` to protect against local file access / exfiltration, as per Warden's journal entry. However, the existing tests in `src/tests/content.test.js` only asserted the *deny* branch (rejecting `javascript:`, `file:`, etc) and lacked tests asserting that the *allow* branch functions correctly for `blob:` and `data:` protocols.
**Action:** Pinned the *allow* behavior in `src/tests/content.test.js` by explicitly verifying that `blob:` and `data:` URLs are correctly accepted by `resolveUrl`.
**Orchestrator note — landed, date corrected from `2026-10-01` (two weeks in the future), and one assertion added on top.** The allow-branch gap was real: four protocols in the allowlist, two of them untested, and `blob:`/`data:` are exactly the two that a future "tighten the allowlist" patch would be tempted to drop. Where the entry stops short is the *shape* of the assertion. `resolveUrl` does not return its input; it returns `parsed.href` — the **normalized** form — and that normalization is itself a security invariant, not an incidental detail: background.js re-parses and forwards `urlObj.href` precisely so the content script and the downloads API cannot disagree about what a string means (the parser differential Warden closed). Your two new cases happen to be fixtures where `href === input`, so they pass identically against a `return url;` implementation that would blow the invariant wide open. A test that cannot fail under the bug it is guarding is a coverage number, not a guard. Added `resolveUrl('HTTP://Example.COM/A.JPG')` → `http://example.com/A.JPG` alongside them, which fails the moment the raw string is passed through. When you pin a function, pick the input that distinguishes its contract from the naive implementation.

## 2026-09-18 - APG Checkbox Naming vs State regression test
**Learning:** Verified that the Pixel persona's fix for correctly applying the APG Checkbox Naming vs State rule (leaving the `aria-label` static and using `aria-checked` to manage state dynamically, while the `title` updates with the action) in the popup lacked a regression test. A drift here could cause accessibility regressions (such as blending state back into the `aria-label`).
**Action:** Added a regression test `tests/popup_toggle.test.js` pinning the toggle button behavior in the popup ensuring that `aria-label` stays static as an identifier, while `title` updates based on checking state and `aria-checked` accurately reflects the state. Exported `wrapCell` from `src/popup.js` to facilitate testing of this behavior.
