...
## 2026-09-12 - Duplicate checking before writing regression tests
**Learning:** Verified that a regression test for `collectInlineSvgs` rejecting `<use>` tags was already present in `src/content.test.js`, despite the Warden persona's journal not mentioning it directly. A test for the fix and its regression guard arrive in the same commit as a rule, so "Warden fixed X" is a strong prior that "X is tested", not a gap to fill.
**Action:** Always follow step 3 of the persona protocol in AGENTS.md, the duplicate check. Perform a thorough `grep` across all test directories (e.g., `grep -rn collectInlineSvgs`) before writing a new test to ensure the coverage doesn't already exist.
