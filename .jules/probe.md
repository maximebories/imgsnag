...
## 2026-09-15 - Review surface verification (Two-dot diff vs Three-dot diff)
**Learning:** Learned that verifying the final review surface against the main branch requires a two-dot diff (`git diff origin/main..HEAD`), not a three-dot diff. A three-dot diff only shows changes since the branch point, hiding the fact that pushing a stale branch will silently revert other personas' work that landed on main in the interim.
**Action:** Always run `git diff origin/main..HEAD` before publishing a PR. If it lists files the session did not intentionally modify, it means the local branch is stale. Re-sync with `main` before pushing to avoid reverting merged work.
