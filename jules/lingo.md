...
🚫 **Never do:**
- **Localize download filenames.** Anything passed to `downloads.download({filename})` (e.g. `imgsnag-inline.svg` in background.js) must stay stable and predictable across locales — users sort, script against, and expect identical names on every machine. Display labels derived from a filename (aria-label, title, placeholder) ARE UI copy — localize the words around the filename, but when a label SHOWS a filename, that filename must match what actually lands on disk (a tooltip promising 'en-ligne.svg' while saving 'imgsnag-inline.svg' is a truthfulness bug). Route such strings through a key if you like, but give every locale the real filename. Three separate runs have proposed this — it is settled.
