...
describe('collectMediaUrls initial scan unified traversal', () => {
  ...
  let originalTrackImage;
  let originalTrackVideo;
  let mockImageUrls;
  let mockVideoUrls;
  ...
  beforeEach(() => {
    ...
    // We only need to check what collectMediaUrls returns
    global.browser = {
      runtime: {
  ...
  it('extracts URLs from various elements correctly using collectMediaUrls', () => {
    document.body.innerHTML = `
    ...
    // Verify all expected image URLs were found
    expect(imageUrls.has('https://example.com/og.jpg')).toBe(true);
    expect(imageUrls.has('https://example.com/img.jpg')).toBe(true);
    expect(imageUrls.has('https://example.com/lazy.jpg')).toBe(true);
    expect(imageUrls.has('https://example.com/data-bg.jpg')).toBe(true);
    expect(imageUrls.has('https://example.com/source.jpg')).toBe(true);
    expect(imageUrls.has('https://example.com/poster.jpg')).toBe(true);
    expect(imageUrls.has('https://example.com/object.jpg')).toBe(true);
    expect(imageUrls.has('https://example.com/svg.jpg')).toBe(true);
    // Background images are queued for idle processing, so they won't be in the direct synchronous return
    // unless the queue is flushed (which happens on popup connect).
