...
3. **Duplicate check — mandatory before writing any patch:**
   ...
   - **A no-op needs no permission.** Do not end your run by asking whether you may conclude, whether the audit looks right, or whether to skip the patch — your charter already answers that, and a question leaves the session parked in `AWAITING_USER_FEEDBACK` until an orchestrator frees it. State the conclusion and stop. The only reason to ask is a genuine fork your definition does not cover.
